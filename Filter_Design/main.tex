...
\author{Sri Krishna Prabhas\\EE23BTECH11224}
\date{}
...
\begin{lstlisting}
https://github.com/yskp123/EE1205/blob/main/Filter_Design/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/yskp123/EE1205/blob/main/Filter_Design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting \eqref{eq:res1} in \eqref{eq:res2} we obtain Gain of required band pass filter:
\begin{align}
    G_{BP} &= 1.0771 
\end{align}
...
A Band-Pass Filter (BPF) can be obtained by subtracting the magnitude response of a Low-Pass Filter (LPF) with cutoff frequency $\omega_{p2}$ from another LPF magnitude response with cutoff frequency $\omega_{p1}$.

\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
    \frac{\sin(\omega_{p1} n)}{n\pi} -\frac{\sin\brak{\omega_{p2}n}}{n\pi},  & \text{for } n\neq 0\\\
    \frac{\omega_{p1}-\omega_{p2}}{\pi} &\text{for } n= 0
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
\begin{align}
     \frac{\sin(\omega_{p1} n)}{n\pi} -\frac{\sin\brak{\omega_{p2}n}}{n\pi} &= \frac{2\cos{\brak{\frac{\omega_{p1}n+\omega_{p2}n}{2}}}\sin{\brak{\frac{\omega_{p1}n-\omega_{p2}n}{2}}}}{n\pi}\\
